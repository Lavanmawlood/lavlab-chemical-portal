...
# ڕێکخستنی لاپەڕەکە بە دیزاینی مۆدێرن
st.set_page_config(
...
st.markdown("<p style='text-align: center; color: #8b949e; font-size: 16px;'>سیستمی پێشکەوتووی کێشانی زانیاری مۆلیکۆڵی بە تێپەڕاندنی بلۆکی سێرڤەر</p>", unsafe_allow_html=True)


def get_request_session() -> requests.Session:
    """دروستکردنی سێشنی پەیوەندی لەگەڵ سیستمی دووبارە تاقیکردنەوەی خۆکار."""
...
def fetch_data_with_proxy_fallback(url: str) -> dict:
    """داواکاری دەنێرێت بۆ PubChem. ئەگەر بلۆک بووبین، خۆکارانە لە ڕێگەی پڕۆکسییەوە دەپەڕێتەوە."""
    session = get_request_session()
    headers = {
        "User-Agent": "Mozilla/5.0 (Windows NT 10.0; Win64; x64) AppleWebKit/537.36 (KHTML, like Gecko) Chrome/120.0.0.0 Safari/537.36"
    }
    
    # تاقیکردنەوەی یەکەم: پەیوەندی ڕاستەوخۆ
    try:
        logging.info(f"هەوڵدانی ڕاستەوخۆ بۆ: {url}")
        response = session.get(url, headers=headers, timeout=6)
        if response.status_code == 200:
            return response.json()
    except Exception as e:
        logging.warning(f"پەیوەندی ڕاستەوخۆ سەرکەوتوو نەبوو: {e}. دەستپێکردنی سیستمی پڕۆکسی...")

    # تاقیکردنەوەی دووەم: بەکارهێنانی پڕۆکسی یەکەم (CORS Proxy)
    try:
        proxy_url_1 = f"https://api.allorigins.win/raw?url={urllib.parse.quote(url)}"
        logging.info(f"هەوڵدان لە ڕێگەی پڕۆکسی AllOrigins بۆ: {proxy_url_1}")
        response = session.get(proxy_url_1, timeout=10)
        if response.status_code == 200:
            return response.json()
    except Exception as e:
        logging.warning(f"پڕۆکسی یەکەم فەشەلی هێنا: {e}")

    # تاقیکردنەوەی سێیەم: بەکارهێنانی پڕۆکسی دووەم (CORS.io)
    try:
        proxy_url_2 = f"https://corsproxy.io/?{urllib.parse.quote(url)}"
        logging.info(f"هەوڵدان لە ڕێگەی پڕۆکسی CorsProxy بۆ: {proxy_url_2}")
        response = session.get(proxy_url_2, timeout=10)
        if response.status_code == 200:
            return response.json()
    except Exception as e:
        logging.error(f"تەواوی سیستمەکان فەشەلیان هێنا لە کارپێکردندا: {e}")
        
    raise ConnectionError("نەتوانرا پەیوەندی بە داتابەیسی فەرمی PubChemـەوە بکرێت بەهۆی بلۆکبوونی توندی سێرڤەرەکەت.")


def get_chemical_info(compound_name: str) -> dict:
    """ناسنامە و زانیارییە گەردییەکان لە ڕێگەی سیستمی پڕۆکسییەوە دەهێنێت."""
    clean_name = urllib.parse.quote(compound_name.strip())
    
    # ١. هێنانی ناسنامەی مۆلیکۆڵ (CID)
    cid_url = f"https://pubchem.ncbi.nlm.nih.gov/rest/pug/compound/name/{clean_name}/cids/JSON"
    cid_data = fetch_data_with_proxy_fallback(cid_url)
    
    if "IdentifierList" not in cid_data or not cid_data["IdentifierList"]["CID"]:
        raise ValueError(f"هیچ پێکهاتەیەک بە ناوی '{compound_name}' نەدۆزرایەوە.")
        
    cid = cid_data["IdentifierList"]["CID"][0]
    
    # ٢. هێنانی تایبەتمەندییە کیمیایییەکان
    ...
    prop_data = fetch_data_with_proxy_fallback(prop_url)
...
if st.button("🚀 Run Molecular Analysis / شیکردنەوەی مۆلیکۆڵی"):
    ...
    if not compound_list:
        ...
        for index, compound in enumerate(compound_list):
            try:
                # کێشانی زانیارییەکان بە شێوازی داینامیکی و بەکارهێنانی پڕۆکسی گۆڕاو
                prop_data = get_chemical_info(compound)
                ...
                # گرنگترین گۆڕانکاری: پیشاندانی هەڵە ڕاستەقینەکە بۆ دیباگکردن
