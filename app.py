...
# ڕێکخستنی سەرەتایی لاپەڕەکە بە دیزاینی مۆدێرن و پان
st.set_page_config(
...
# لیستی ناسنامەی جیاوازی وێبگەڕەکان (User-Agents)
USER_AGENTS = [
...
def scrape_pubchem_live(url: str) -> dict:
    ...
    # ٢. هەوڵدانی دووەم بە بەکارهێنانی پڕۆکسی AllOrigins
    try:
...
if st.button("🚀 Run Molecular Analysis / شیکردنەوەی مۆلیکۆڵی"):
    ...
    if not compound_list:
        ...
        for index, compound in enumerate(compound_list):
            try:
                # کێشانی ڕاستەوخۆ بە لۆجیکی سکراپینگ
