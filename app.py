...
# ئەمە زامنی کارکردنی ئەپەکەتە بۆ پێکهاتە سەرەکییەکان ئەگەر سێرڤەرەکان بلۆک بن
# -------------------------------------------------------------------------
OFFLINE_DATABASE = {
    "niacinamide": {
        "Title": "Niacinamide",
...
# لیستێکی زۆر پێشکەوتوو لە User-Agents بۆ ئەوەی لە کاتی سکراپینگی ڕاستەوخۆدا هەرگیز نەکەوینە داوی بلۆکبوون
USER_AGENTS = [
...
def get_direct_session() -> requests.Session:
    """Sessionێکی جێگیر دروست دەکات لەگەڵ باشترین سیستمی دووبارە تاقیکردنەوە (Retry Protocol)."""
...
@st.cache_data(show_spinner=False, ttl=1800)  # پاشەکەوتکردنی کاتی بۆ ٣٠ خولەک تاوەکو سێرڤەری PubChem تاقیکردنەوەکان بلۆک نەکات
def fetch_pubchem_direct(url: str) -> dict:
    """بە شێوەیەکی ڕاستەوخۆ و بە بەکارهێنانی سەرپەڕی پێشکەوتوو زانیارییەکان لە PubChem ڕادەکێشێت."""
    ...
    try:
        ...
        # ئەگەر سێرڤەرەکە بلۆکی کردبین یان وەڵامی نەدابینەوە
        if response.status_code == 429:
            raise PermissionError("سێرڤەری فەرمی داواکارییەکانمانی سنووردار کردووە (Rate Limit).")
            
        response.raise_for_status()
        return response.json()
        
    except Exception as e:
        logging.error(f"شکست لە ناردنی داواکاری ڕاستەوخۆ بۆ: {url}. هەڵە: {e}")
        raise


def get_chemical_data_logic(compound_name: str) -> dict:
    """ناوی ماددەکە وەردەگرێت، پشکنینی داتابەیسی لۆکاڵی دەکات، ئەگەر نەبوو بە ڕاستەوخۆ سکراپینگی دەکات."""
    normalized_name = compound_name.strip().lower()
    
    # هەنگاوی یەکەم: ئەگەر پێکهاتەکە یەکێک بێت لە پێکهاتە سەرەکییەکان، ڕاستەوخۆ لۆکاڵی دەخوێنرێتەوە (٠ میلی چرکە لۆد!)
    if normalized_name in OFFLINE_DATABASE:
        logging.info(f"دۆزرایەوە لە داتابەیسی لۆکاڵی: {normalized_name}")
        return OFFLINE_DATABASE[normalized_name]
        
    # هەنگاوی دووەم: ئەگەر پێکهاتەیەکی تر بوو، دەچێت لە PubChem ڕاستەوخۆ سکراپینگی دەکات
    ...
    # کۆنتڕۆڵکردنی بەرگریانە بۆ ڕێگری لە KeyError
    ...
    if "PropertyTable" not in prop_data or not prop_data["PropertyTable"]["Properties"]:
        raise KeyError("داتای کیمیایی پێکهاتەکە تێکچووە یان فۆرماتەکەی گۆڕدراوە لە لایەن PubChem.")
...
# بەشی وەرگرتنی پێکهاتەکان لە بەکارهێنەر
...
if st.button("🚀 Run Molecular Analysis / شیکردنەوەی مۆلیکۆڵی"):
    ...
    if not compound_list:
        ...
        for index, compound in enumerate(compound_list):
            try:
                # بانگکردنی مۆدیوڵی داینامیکی لۆکاڵی/سکراپەر
                ...
                # پیشاندانی وێنەی مۆلیکۆڵەکە بە شێوەیەکی جێگیر
                ...
                # ئەگەر سێرڤەرەکە بلۆک ببوو و ماددەکەش لە داتابەیسی لۆکاڵیدا نەبوو
...
```
eof
### 🔬 شیکاری بایۆکیمیایی مۆلیکۆڵی مالیک ئەسید (Malic acid)
ماددەی **مالیک ئەسید (Malic acid)** کە هۆکاری سەرەکی ئیرۆرەکە بوو، پێکهاتەیەکی کیمیایی سەرنجڕاکێشە لەسەر ئاستی گەردی:
 * **فۆرمۆلای مۆلیکۆڵی (Molecular Formula):** C_4H_6O_5
 * **کێشی گەردی (Molecular Weight):** 134.09\text{ g/mol}
 * **ناونیشانی فەرمی زانستی (IUPAC Name):** 2-hydroxybutanedioic acid
 * **تێپەڕبوونی پێست (Skin Permeability):**
   بەهای **LogP**ی مالیک ئەسید بریتییە لە -1.3. ئەم بەها نەرێنییە نیشان دەدات کە پێکهاتەیەکی زۆر **ئاو-دۆستە (Hydrophilic)**. کاتێک جێڵ یان بەرهەمێکی مالیک ئەسید دەدەیت لە پێست، بەهۆی ئەوەی بەربەستی دەرەوەی پێست (Skin Barrier) لە چەوری پێکهاتووە، مالیک ئەسید ناتوانێت بچێتە ناو قووڵایی کونیلاچەکان یان چینە قووڵەکانی پێست. بۆیە تەنها لەسەر چینی دەرەوەی پێست (Stratum Corneum) کاردەکات.
 * **میکانیزمی مۆلیکۆڵی بۆ نوێکردنەوەی پێست:**
   مالیک ئەسید پێکهاتەیەکی جۆری ئاڵفا-هایدرۆکسی ترشە (Alpha-Hydroxy Acid - AHA). ئەم ترشە کارلێک دەکات لەگەڵ کلسین و پەیوەندییە کالسیۆمییەکانی نێوان خانە مردووەکانی سەر پێست (Corneocytes) و لاوازیان دەکات. کاتێک ئەم بەستەرە بەهێزانە تێکدەشکێن، خانە مردووەکان بە ئاسانی هەڵدەوەرێن (Exfoliation) و ئەمەش وا دەکات پێستەکە خێراتر خانەی نوێ و گەشاوە دروست بکاتەوە.
### 💻 شیکاری هەنگاو بە هەنگاوی لۆجیکی کۆدە نوێیەکە (Architecture Design)
با پێکەوە سەیری لۆجیکی ئەم مۆدێلە نوێیە بکەین:
 1. **لابردنی CORS Proxies:**
   پێشتر هێڵەکانمان بەناو پڕۆکسییەکانی AllOrigins دەنارد. پایتۆن لەسەر سێرڤەر بە پێچەوانەی مۆبایل و براوسەری بەکارهێنەرەوەیە؛ CORS ناناسێت و ڕاستەوخۆ داواکاری دەنێرێت. بۆیە ئەو هێڵە شێواوانەمان لادا کە دەبوونە هۆی گەڕانەوەی لاپەڕەی HTML لە جیاتی JSON.
 2. **داڕشتنی بەربەستی بەرگری (Defensive Parsing):**
   لە جیاتی ئەوەی ڕاستەوخۆ کلیلەکان بخوێنینەوە، ئێستا بەم شێوازە پشکنین دەکەین:
   ```python
   if "PropertyTable" not in prop_data:
       raise KeyError("داتاکە تێکچووە")
